...
class SupaSet:
        ...
        if autoexpire:
            if isinstance(autoexpire, int):
                ...
                data = (
                    ...
                    .lt("created_at", date_days_ago)
                    .execute()
                )
                logger.info(
                    f"Deleted entries older than {autoexpire} days {date_days_ago} for set {self.set_name}; data: {data}"
                )
            else:
    ...
    def remove(self, value):
        ...
        try:
            self._table().delete().eq("value", value).eq(
                "session_id", str(self.session_id)
            ).eq("set_name", self.set_name).execute()
        except Exception as e:
    ...
    def complete_session(self):
        try:
            ...
            logger.info(
                f"Completed session {self.session_id} for supaset {self.set_name} with {len(self.elements())} elements."
            )
        except Exception as e:
    ...
    def _cleanup_incomplete_sessions(self):
        try:
            # Delete all incomplete sessions except for the current session (for this set_name)
            self._table().delete().eq("is_session_complete", False).eq(
                "set_name", self.set_name
            ).neq("session_id", str(self.session_id)).execute()
        except Exception as e:
            logger.error(f"Failed to delete incomplete session entries, error: {e}")
