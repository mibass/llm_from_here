...
def test_search_video_with_duration(yt_fetch, chat_app, enforce_json_prompt_template):
    query = 'Bill Burr'
    min_duration = 300
    max_duration = 660
    duration_search_filter = "medium"
    additional_query_text = '("stand up"|"live from here")'
    llm_filter_prompt = enforce_json_prompt_template.format(
        "{{ title }}", "{{ channel_title }}", "{{ description }}"
    )

    # Call the function being tested
    result = yt_fetch.search_video_with_duration(
        query=f"{query} {additional_query_text}",
