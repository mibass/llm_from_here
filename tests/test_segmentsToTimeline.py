...
from segmentsToTimeline import SegmentsToTimeline
...
class TestSegmentsToTimeline(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.stt = SegmentsToTimeline(self.mock_params, self.mock_global_results, self.mock_plugin_instance_name)

    @patch('showTTS.ShowTextToSpeech')
    ...
    @patch("segmentsToTimeline.generate_applause")
    def test_applause_generator(self, mock_generate_applause):
        test_text = "duration 10"
        test_output_file = "test.wav"
        self.stt.applause_generator(test_text, test_output_file)
        mock_generate_applause.assert_called_once()

    @patch('freesoundfetch.FreeSoundFetch.search_and_download_top_samples')
    @patch("segmentsToTimeline.os.path.dirname")
    @patch("segmentsToTimeline.shutil.move")
    ...
    def test_get_transition_map_entry(self):
        with patch.object(self.stt.timeline, 'get_last_type', return_value='music') as mock_get_last_type:
            result = self.stt.get_transition_map_entry(self.mock_segment_transition_map, 'audience')
    ...
    def test_generate_audio_segments(self):
        ...
        self.stt.generate_audio_segments(data, output_folder, self.mock_params, self.mock_plugin_instance_name)

        # Assert that the expected methods were called with the correct arguments
        self.stt.applause_generator.assert_called_once_with('dialog2', os.path.join(output_folder, 'test_instance_002.wav'))
        self.stt.fast_TTS.assert_called_once_with('dialog1', os.path.join(output_folder, 'test_instance_001.wav'))
        self.stt.youtube_playlist.assert_called_once()

        # Clean up the temporary directory
        shutil.rmtree(output_folder)
        
    @patch('audioTimeline.AudioTimeline')
